...
var numberOfContests = -3; //variable to store the number of contestants
...
function init() {
    ...
    dojo.byId("txtAddress").setAttribute("defaultAddress", responseObject.LocatorSettings.Locators[0].LocatorDefaultAddress);
    dojo.byId("txtAddress").setAttribute("defaultPrecinct", responseObject.LocatorSettings.Locators[1].LocatorDefaultPrecinct);
    dojo.byId("txtAddress").value = responseObject.LocatorSettings.Locators[0].LocatorDefaultAddress;
    lastSearchString = dojo.byId("txtAddress").value.trim();
    ...
    dojo.connect(pLayer, "onClick", function (evt) {
        if (!isMobileDevice) {
            if (currentSelectedLayer) {
                ShowInfoWindow(evt.mapPoint, evt.graphic.attributes[precinctLayer.PrecinctName],true);
            }
            dojo.stopEvent(evt);
        }
    });
    ...
    if (isMobileDevice) {   //If mobile device, list items are created
        ...
        for (var index in electionResultData) {
            ...
            dojo.connect(itemWidget.domNode, "onclick", function (e) {
                ...
                FetchContestData(electionResultData[key], key, null, map.getLayer(precinctLayer.Key).getSelectedFeatures()[0].attributes[precinctLayer.PrecinctName], true);
                dojo.byId("tdList").style.display = "block";
                dojo.byId("tdListHeader").innerHTML = "Precinct: " + map.getLayer(precinctLayer.Key).getSelectedFeatures()[0].attributes[precinctLayer.PrecinctName];
